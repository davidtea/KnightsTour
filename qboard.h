...
public:
    ...
    vector<coord> checkValidSpots(const coord &c); //used in generate tours, finds moves without endpos
    ...
    bool validSpot(const coord &c); //if it exists on the qBoard and haven't been there or tried yet and not the end pos unless it's the end
    ...
    void setHistory(const coord &c); //records the knights sequence of moves around the qBoard
    ...
    void print(); //prints the qBoard

    friend
    ostream& operator<<(ostream &out, const qBoard &b); //chess notation

private:
    int XDIM; //x dimension of qBoard
    int YDIM; //y dimension of qBoard
    ...
    pQueue<coord,int> moves; //stack of moves knight has made
};
